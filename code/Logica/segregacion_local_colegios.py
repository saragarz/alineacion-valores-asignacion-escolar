...
def marcador_segregacion_colegio(segregacion_colegio, min_segregacion, max_segregacion, coordenadas_colegios):
    ...
    popup = (f"{nombre}<br>"
             f"Municipio: {segregacion_colegio['municipio']}<br>"
...
segregacion_colegios = logica_lib.RunPredicateToPandas('segregacion.l',
                                                       'SegregacionMunicipal', connection=conexion)
...
todos_municipios = folium.FeatureGroup(name="Todos los municipios", show=False)
marcadores_municipio = {}

for _, segregacion_colegio in segregacion_colegios.iterrows():
    muni = segregacion_colegio['municipio']
    if muni not in marcadores_municipio:
        marcadores_municipio[muni] = folium.FeatureGroup(name=f'{muni}', show=False)

    max_segregacion_muni = max(segregacion_colegios[segregacion_colegios['municipio'] == muni]['valor_segregacion'])
    min_segregacion_muni = min(segregacion_colegios[segregacion_colegios['municipio'] == muni]['valor_segregacion'])

    marcadorGlobal = marcador_segregacion_colegio(segregacion_colegio, min_segregacion_muni, max_segregacion_muni, coordenadas_colegios)
    marcadorMunicipio = marcador_segregacion_colegio(segregacion_colegio, min_segregacion_muni, max_segregacion_muni, coordenadas_colegios)

    marcadorGlobal.add_to(todos_municipios)
    marcadorMunicipio.add_to(marcadores_municipio[muni])

    cont += 1
    print(cont)

todos_municipios.add_to(mapa)

for nombre_muni in sorted(marcadores_municipio.keys()):
    colegios_municipio = marcadores_municipio[nombre_muni]
    colegios_municipio.add_to(mapa)

folium.LayerControl(collapsed=False).add_to(mapa)

mapa.save(f"../../maps/segregacion_local_colegios.html")
